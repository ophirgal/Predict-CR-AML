...
''' Anotating age of Diagnosis '''
for i in range(len(pca_df)):
    y = int(int(clinical_df['Age at Diagnosis in Days'][pca_df.index.values[i]])/1000)
    
    if cr_ncr_df.index.values[i] in cr_df.index.values:
        ax.scatter(pca_df.PC1.loc[cr_ncr_df.index.values[i]],
               pca_df.PC2.loc[cr_ncr_df.index.values[i]],
               pca_df.PC3.loc[cr_ncr_df.index.values[i]],
               c=colors[y], marker='o')
    else:
        ax.scatter(pca_df.PC1.loc[cr_ncr_df.index.values[i]],
                   pca_df.PC2.loc[cr_ncr_df.index.values[i]],
...
plt.title('PCA: CR vs N_CR (Diff Age at Diagnosis)')
...
for i in range(len(pca_df)):
    y = int(clinical_df['Year of Diagnosis'][pca_df.index.values[i]])
...
plt.title('PCA: CR vs N_CR (Diff Year at Diagnosis)')
