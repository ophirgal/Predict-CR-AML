...
all_scores = {5:[], 10:[], 15:[], 30:[], 40:[], 50:[], 100:[]}
all_labels = {5:[], 10:[], 15:[], 30:[], 40:[], 50:[], 100:[]}
...
for i in range(N_ITERATIONS):
    ...
    rf = RandomForestClassifier(n_estimators=N_TREES, 
                                max_features=int(np.sqrt(len(X_sub_train.columns))),
                                max_depth=int(np.sqrt(len(X_sub_train.columns))),
                                random_state=42 + i)
    rf.fit(X_sub_train, [1 if y == 'CR' else 0 for y in y_sub_train.values])
    
    gene_importances = dict(zip(X_sub_train.columns, rf.feature_importances_))
    genes_by_importance = sorted(gene_importances, reverse=True, 
                                 key=lambda x: gene_importances[x])     
    for n_final_genes in [5, 10, 15, 30, 40, 50, 100]:        
...
for n_final_genes in [5, 10, 15, 30, 40, 50, 100]:  
    ...
    ''' Plotting '''
    plt.plot(fpr, tpr, label = 'ֵֵֵֵtop %d genes, AUC = %0.2f' \
             % (n_final_genes, roc_auc))

''' -------------------- DISPLAYING FINAL FIGURE ------------------------- '''    
plt.title('ROC Curve for Random Forest (%d trees, simple FS, 100 reps)' % N_TREES)
